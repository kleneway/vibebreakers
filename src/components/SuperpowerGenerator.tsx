...
export const SuperpowerGenerator: React.FC<SuperpowerGeneratorProps> = ({
  ...
  const removeSuperpowerAssignment = (superpowerId: string) => {
    const updatedSuperpowers = selectedSuperpowers.filter(
      (sp) => sp.id !== superpowerId,
    );
    setSelectedSuperpowers(updatedSuperpowers);
    onSuperpowersUpdate(updatedSuperpowers);
  };
    ...
    <div className="space-y-6">
      <div className="bg-purple-50 dark:bg-purple-900/20 p-6 rounded-lg">
        ...
        <p className="text-purple-800 dark:text-purple-200 mb-4">
          Identify one invisible superpower for each person. Choose creative
          names that capture their unique strengths!
        </p>

        <div className="bg-purple-100 dark:bg-purple-800/30 p-4 rounded-lg">
          <p className="text-purple-800 dark:text-purple-200 font-medium">
            🎯 <strong>{currentPlayer?.name}</strong> - Choose superpowers for
            others
          </p>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          {otherPlayers.map((player) => {
            const existingAssignment = selectedSuperpowers.find(
              (sp) =>
                sp.playerId === player.id && sp.assignedBy === currentPlayer.id,
            );
                ...
                {existingAssignment ? (
                  <div className="bg-green-50 dark:bg-green-900/20 p-3 rounded-lg">
                    <div className="flex justify-between items-center">
                      ...
                      <Button
                        onClick={() =>
                          removeSuperpowerAssignment(existingAssignment.id)
                        }
                        variant="secondary"
                        size="sm"
                      >
              ...
              <div className="grid grid-cols-1 gap-1">
                {category.powers.map((power) => (
                  <button
                    key={power}
                    onClick={() => {
                      const unassignedPlayer = otherPlayers.find(
                        (p) =>
                          !selectedSuperpowers.some(
                            (sp) =>
                              sp.playerId === p.id &&
                              sp.assignedBy === currentPlayer.id,
                          ),
                      );
                      if (unassignedPlayer) {
