...
export const EmpathyAnalyzer: React.FC<EmpathyAnalyzerProps> = ({
  ...
  const calculateAccuracy = (): PredictionAccuracy[] => {
    ...
    predictions.forEach((prediction) => {
      const actualResponse = responses.find(
        (r) => r.playerId === prediction.targetPlayerId,
      );

      if (actualResponse) {
        // Simple similarity scoring based on key words and sentiment
        const score = calculateSimilarityScore(
          prediction.predictedResponse,
  ...
  const calculateSimilarityScore = (
    ...
    const overlapScore =
      (matches / Math.max(predictionWords.length, actualWords.length)) * 100;
    const lengthSimilarity =
      Math.min(prediction.length, actual.length) /
      Math.max(prediction.length, actual.length);
  ...
  const generateInsights = (accuracy: PredictionAccuracy[]) => {
    ...
    const mostSurprising =
      surprisingResults.length > 0
        ? surprisingResults[0].targetPlayerId
        : participants[0];
      ...
      <div className="bg-gradient-to-r from-purple-500 to-pink-500 p-6 text-white">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-white/80">See how well you understood others</p>
          </div>
                ...
                {analysis.accuracy.map((result, index) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getAccuracyBadge(
                ...
                <div className="space-y-4">
                  <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-6">
                    ...
                    <p className="text-green-700 dark:text-green-300">
                      <strong>{analysis.insights.bestPredictor}</strong> showed
                      the highest average accuracy in understanding others.
                    </p>
                  </div>
                  <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-6">
                    ...
                    <p className="text-yellow-700 dark:text-yellow-300">
                      <strong>{analysis.insights.mostSurprising}</strong>'s
                      response was the most unexpected to others.
                    </p>
                  </div>
                </div>
            ...
            >
              ...
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl p-6">
                ...
                <div className="space-y-4 text-slate-700 dark:text-slate-300">
                  <p>
                    <strong>What this means:</strong> Understanding others'
                    decision-making is complex and requires looking beyond
                    surface-level assumptions.
                  </p>
                  <p>
                    <strong>Key learning:</strong> Even people we know well can
                    surprise us with their responses to ethical and personal
                    dilemmas.
                  </p>
                  <p>
                    <strong>Moving forward:</strong> Use these insights to ask
                    better questions and listen more deeply to understand
                    others' perspectives.
                  </p>
                </div>
              </div>
