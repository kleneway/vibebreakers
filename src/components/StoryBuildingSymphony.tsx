...
export const StoryBuildingSymphony: React.FC<StoryBuildingSymphonyProps> = ({
  ...
  const initializePlayers = useCallback(() => {
    ...
    const elements = elementGenerator.generateElementsForPlayers(
      playerNames.length,
    );
  ...
  const handleContribution = useCallback(
    (sentence: string) => {
      ...
      const lengthBonus = Math.min(
        5,
        Math.floor((sentence.length - settings.minSentenceLength) / 20),
      );
      const integrationBonus = sentence
        .toLowerCase()
        .includes(currentPlayer.assignedElement.text.toLowerCase())
        ? 5
        : 0;
      ...
      if (isGameComplete) {
        ...
        toast.success(
          `Great addition ${currentPlayer.name}! +${totalScore} points`,
        );
      }
    },
  ...
  // Export story
  const exportStory = () => {
    const storyText = gameState
      .story!.map(
        (contribution, index) =>
          `${index + 1}. ${contribution.playerName} (${
            contribution.storyElement.text
          ...
          <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900 rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm text-blue-800 dark:text-blue-200">
              ...
              <div>
                ...
                {settings.maxSentenceLength} chars
              </div>
            </div>
          </div>
  ...
  if (gameState.status === "finished") {
    ...
    return (
      <div className="w-full max-w-4xl mx-auto space-y-6">
        ...
        {/* Final Story */}
        <StoryPromptCard story={gameState.story!} onExportStory={exportStory} />
