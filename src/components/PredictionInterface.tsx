...
export const PredictionInterface: React.FC<PredictionInterfaceProps> = ({
  ...
  React.useEffect(() => {
    if (
      ...
      currentTarget < otherParticipants.length
    ) {
  ...
  const handleSubmitAll = async () => {
    ...
    const playerPredictions: PlayerPrediction[] = finalPredictions.map(
      (pred) => ({
  ...
  const isCurrentPredictionComplete =
    currentPrediction.predictedResponse.trim().length > 0;
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-sm text-slate-500 dark:text-slate-400">
              {Math.round(
                ((currentTarget + (isCurrentPredictionComplete ? 1 : 0)) /
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="mt-2 text-center">
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-aurora-100 text-aurora-800 dark:bg-aurora-900 dark:text-aurora-200">
                        {currentPrediction.confidence <= 3
                          ? "Not very confident"
                          : currentPrediction.confidence <= 6
                          ? "Somewhat confident"
                          : currentPrediction.confidence <= 8
                          ? "Quite confident"
                          : "Very confident"}
                      </span>
                    </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        handlePredictionChange("reasoning", e.target.value)
                      }
                  ...
                  <div className="text-sm text-slate-500 dark:text-slate-400">
                    {predictions.length > 0 && (
                      <span>
                        ...
                        {predictions.length !== 1 ? "s" : ""} completed
                      </span>
                    )}
                  </div>
