...
export function InvisibleSuperpowers({ players, onGameComplete }: InvisibleSuperpowersProps) {
          ...
          <div className="text-center space-y-6">
            <div className="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-lg">
              ...
              <p className="text-blue-800 dark:text-blue-200 mb-4">
                Discover and celebrate the subtle but powerful strengths that make each person unique.
              </p>
              <div className="text-left space-y-2 text-sm text-blue-700 dark:text-blue-300">
                <p><strong>Phase 1:</strong> Observe others during casual conversation (3 min)</p>
                <p><strong>Phase 2:</strong> Identify invisible superpowers (2 min)</p>
                <p><strong>Phase 3:</strong> Gather evidence for each superpower (1.5 min)</p>
                <p><strong>Phase 4:</strong> Superpower bestowal ceremony (3 min per person)</p>
                <p><strong>Phase 5:</strong> Reflection and integration (3 min)</p>
              </div>
            </div>
            <Button onClick={startPhase} className="bg-blue-600 hover:bg-blue-700">
              Begin Discovery
            </Button>
          </div>
        );
          ...
          <div className="space-y-6">
            <div className="bg-green-50 dark:bg-green-900/20 p-6 rounded-lg">
              ...
              <div className="space-y-4 text-green-800 dark:text-green-200">
                ...
                <ul className="space-y-2 text-sm">
                  ...
                  <li>• Which superpowers complement each other in the group?</li>
                  <li>• What patterns do you notice across all the superpowers?</li>
                </ul>
              </div>
            </div>
            ...
            <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg">
              ...
              <div className="grid gap-2 text-sm">
                {players.map(player => {
                  const playerSuperpowers = superpowers.filter(sp => sp.playerId === player.id);
                  return (
                    <div key={player.id} className="text-purple-800 dark:text-purple-200">
                      <strong>{player.name}:</strong> {playerSuperpowers.map(sp => sp.name).join(", ")}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="text-center space-y-6">
            <div className="bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 p-8 rounded-lg">
              ...
              <div className="text-left space-y-3">
                {players.map(player => {
                  const playerSuperpowers = gameResults.filter(sp => sp.playerId === player.id);
                  return (
                    <div key={player.id} className="bg-white dark:bg-gray-800 p-4 rounded-lg">
                      <h4 className="font-bold text-purple-900 dark:text-purple-100">
                        {player.name}'s Superpowers:
                      </h4>
                      {playerSuperpowers.map(sp => (
                        <div key={sp.id} className="text-sm text-purple-700 dark:text-purple-300 mt-1">
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="flex justify-center space-x-2">
        {(["setup", "observation", "identification", "evidence", "ceremony", "reflection"] as GamePhase[]).map((phase, index) => (
          <div
            key={phase}
            className={`w-3 h-3 rounded-full ${
              currentPhase === phase
                ? "bg-blue-600"
                : ["setup", "observation", "identification", "evidence", "ceremony", "reflection"].indexOf(currentPhase) > index
                ? "bg-green-600"
      ...
      {currentPhase !== "setup" && currentPhase !== "complete" && (
        <div className="flex justify-center space-x-4">
          {!isTimerActive && (
            <Button onClick={startPhase} className="bg-green-600 hover:bg-green-700">
              Start Phase
            </Button>
          )}
