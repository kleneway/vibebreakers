...
export default function TwoTruthsAndLie() {
  ...
  const [gamePhase, setGamePhase] = useState<'setup' | 'statements' | 'guessing' | 'results'>('setup');
  ...
  const submitStatements = () => {
    if (currentStatements.every((s: string) => s.trim()) && lieIndex !== null) {
      const statements: Statement[] = currentStatements.map((text: string, index: number) => ({
        id: index,
        text: text.trim(),
  ...
  const makeGuess = (statementId: number) => {
    ...
    const targetPlayer = players.find((p: Player) => p.statements.some((s: Statement) => s.id === statementId));
  ...
  const getOtherPlayers = () => players.filter((_, index) => index !== currentPlayerIndex);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900 dark:to-pink-900 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        {gamePhase === 'statements' && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-4">
              Write two true statements and one lie. Other players will try to guess which is the lie!
            </p>
              ...
              {currentStatements.map((statement, index) => (
                <div key={index} className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor={`lie-${index}`} className="text-sm text-gray-600 dark:text-gray-300">
                      This is the lie
                    </label>
                  </div>
                </div>
            ...
            <Button
              ...
              disabled={!currentStatements.every(s => s.trim()) || lieIndex === null}
              className="w-full"
            >
              Submit Statements
            </Button>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
              ...
              {getOtherPlayers().map((player) => (
                <div key={player.id} className="mb-6 p-4 border border-gray-200 dark:border-gray-600 rounded-lg">
                  <h3 className="text-lg font-semibold mb-3">{player.name}'s statements:</h3>
                  <div className="space-y-2">
        ...
        {gamePhase === 'results' && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
            <h2 className="text-2xl font-semibold mb-6 text-center">Game Results! 🎉</h2>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {player.score > 0 && <CheckCircle className="w-5 h-5 text-green-500" />}
                      </div>
                    </div>
                  </div>
                ))}
